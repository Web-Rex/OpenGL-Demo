...
int main()
{
    ...
    EVENTS::Add_Keys_Event([&glfw_window](int key, unsigned int *action)
        ...
        if (action[KEY_ESCAPE] == KEY_PRESS)
            glfw_window.Close_Window(); }, "Close_Window");
    ...
    Light light(shader1,
        0, 1, 64.0f,             // material.diffuse | material.specular | material.shininess
        camera.Get_Camera_Pos()  // viewPos
    );
    ...
    glm::vec3 pLPos = glm::vec3(0.7f, 2.2f, 2.0f);
    ...
    while (!glfw_window.Get_Window_Should_Close())
    {
        ...
        // [...] directional light
        light.Direction_Light(shader1,
            glm::vec3(-0.2f, -1.0f, -0.3f), // dirLight.direction
            glm::vec3(0.05f, 0.05f, 0.05f), // dirLight.ambient
            glm::vec3(0.8f, 0.8f, 0.8f),    // dirLight.diffuse
            glm::vec3(1.0f, 1.0f, 1.0f)     // dirLight.specular
        );

        // [...] point light 1
        // light.Point_Light(shader1,
        //     glm::vec3(pLPos.x, pLPos.y, pLPos.z), // pointLights.position
        //     glm::vec3(0.05f, 0.05f, 0.05f),       // pointLights.ambient
        //     glm::vec3(0.8f, 0.8f, 0.8f),          // pointLights.diffuse
        //     glm::vec3(1.0f, 1.0f, 1.0f),          // pointLights.specular
        //     1.0f, 0.09f, 0.032f                   // constant | linear | quadratic
        // );

        // [...] spotLight
        // light.Spot_Light(shader1,
        //     glm::vec3(lightPos.x, lightPos.y, lightPos.z),                                              // spotLight.position
        //     glm::vec3(camera.Get_Camera_Dir().x, camera.Get_Camera_Dir().y, camera.Get_Camera_Dir().z), // spotLight.position
        //     glm::vec3(0.0f, 0.0f, 0.0f),                                                                // spotLight.direction
        //     glm::vec3(1.0f, 1.0f, 1.0f),                                                                // spotLight.ambient
        //     glm::vec3(1.0f, 1.0f, 1.0f),                                                                // spotLight.diffuse
        //     1.0f, 0.09f, 0.032f,                                                                        // constant | linear | quadratic
        //     glm::cos(glm::radians(12.5f)),                                                              // spotLight.cutOff
        //     glm::cos(glm::radians(15.0f))                                                               // spotLight.outerCutOff
        // );
        ...
        //
        trans = glm::mat4(1.0f);
        trans = glm::translate(trans, pLPos);
        trans = glm::scale(trans, glm::vec3(0.05, 0.05, 0.05));
